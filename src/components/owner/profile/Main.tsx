...
import {AlertType, ColorButton, Gender, GenderLabel, Role} from "@/types/enum";
...
export default function Main() {
  ...
  return (
    <div className={"flex flex-col md:flex-row gap-6 p-8 overflow-y-auto"}>
      {isLoading && <Loading/>}
      {/* Left Section */}
                      ...
                      <div className="text-sm text-grey-c600">
                        {!isSupported
                          ...
                          : permission === 'denied'
                            ? "Bạn đã từ chối quyền thông báo. Vui lòng bật lại trong cài đặt trình duyệt."
                            : isSubscribed
                              ? "Bạn đang nhận thông báo về đơn hàng và tin tức mới"
                              : "Bật để nhận thông báo về đơn hàng và tin tức mới"
                        }
                      </div>
                    </div>
                  </div>
      ...
      {isOpen[1] && <ChangePassword isOpen={isOpen[1]}
                                        setIsOpen={() => {
                                          setIsOpen(prevState => {
                                            const newState = [...prevState];
                                            newState[1] = false;
