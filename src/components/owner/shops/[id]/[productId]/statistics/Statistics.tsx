...
export default function Statistics({product}: Props) {
  ...
  return (
    <div className="py-6 space-y-6">
      ...
      {/* Variant Performance */}
      <Card>
        <h3 className="text-xl font-bold mb-6 text-grey-c900">Hiệu suất biến thể</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b text-left">
                <th className="py-3 px-2 text-sm font-semibold text-grey-c700">Biến thể</th>
                <th className="py-3 px-2 text-sm font-semibold text-grey-c700">Giá</th>
                <th className="py-3 px-2 text-sm font-semibold text-grey-c700">Đã bán</th>
                <th className="py-3 px-2 text-sm font-semibold text-grey-c700">Kho</th>
                <th className="py-3 px-2 text-sm font-semibold text-grey-c700">Doanh thu</th>
                <th className="py-3 px-2 text-sm font-semibold text-grey-c700">% Tổng doanh thu</th>
              </tr>
            </thead>
            <tbody>
              {product.productVariants
                ...
                .map((variant) => {
                  const variantRevenue = variant.price * variant.sold;
                  const revenuePercentage = totalRevenue > 0
                    ? ((variantRevenue / totalRevenue) * 100).toFixed(1)
                    : "0.0";

                  return (
                    <tr key={variant.productVariantId} className="border-b hover:bg-grey-c50">
                      <td className="py-3 px-2 text-sm">
                        {variant.productVariantAttributeValues.map((av) => {
                          ...
                          return val?.productAttributeValue;
                        }).join(" - ")}
                      </td>
                      <td className="py-3 px-2 text-sm font-semibold">{formatPrice(variant.price)}</td>
                      <td className="py-3 px-2 text-sm">{variant.sold}</td>
                      <td className="py-3 px-2 text-sm">
                        <span className={variant.stockQuantity < 10 ? 'text-support-c800 font-semibold' : ''}>
                          {variant.stockQuantity}
                        </span>
                      </td>
                      <td className="py-3 px-2 text-sm font-semibold text-success-c700">
                        {formatPrice(variantRevenue)}
                      </td>
                      <td className="py-3 px-2 text-sm">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-grey-c200 rounded-full h-2 overflow-hidden max-w-[100px]">
                            <div
                              className="bg-primary-c600 h-full"
                              style={{ width: `${revenuePercentage}%` }}
                            />
                          </div>
                          <span className="text-xs">{revenuePercentage}%</span>
                        </div>
                      </td>
                    </tr>
                  );
              ...
              {product.productVariants.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-grey-c600">
