...
export default function OrderItemDetailModal({
  ...
  } = useSWR(`${REVIEW_VIEW}/${selectedOrderItem.orderItemId}`, fetcherGetReview, {
    refreshInterval: 0,
        ...
        <div>
            {isReviewLoading ? (
              ...
              <div className="space-y-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {existingReview.imageUrls.map((url, idx) => (
                      <button key={idx} type="button"
                              className="w-20 h-20 rounded-lg overflow-hidden border-2 border-grey-c200">
                        <Image src={url} alt={`review-${idx}`} width={80} height={80}
                ...
                {existingReview.reviewReplyView && (
                  <div className="bg-primary-c50 rounded-lg p-3 mt-3 border-l-4 border-primary-c400">
                  ...
                  <Controller
                    ...
                    render={({field}) => (
                      <div className="flex items-center gap-2 justify-center">
                        <div>{(Object.values(RatingNumber).filter(v => typeof v === 'number') as number[]).map((star) => (
                          <button
                            key={star}
                            type="button"
                          ...
                          >
                            {(hoveredRating ?? field.value ?? 0) >= star ? (
                              ...
                              <StarBorderRoundedIcon fontSize="large" className="text-yellow-500"/>
                            )}
                          </button>
                        ))}</div>
